...
public class ClaimTradesUI {
    ...
    public static void openUI(EnvyPlayer<EntityPlayerMP> player, int page) {
        GuiConfig.ClaimTradesConfig config = EnvyGTSForge.getInstance().getGui().getClaimTradesUIConfig();
        ...
        List<Trade> allTrades = EnvyGTSForge.getInstance().getTradeManager().getPurchasedTrades(player);
        ...
        for (int i = (page * 45); i < ((page + 1) * 45); i++) {
            ...
            trade.displayClaimable(i % 45, clicker -> openUI((EnvyPlayer<EntityPlayerMP>) clicker, page), pane);
        }
