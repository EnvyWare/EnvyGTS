...
public class ReforgedGTSQueries {
    ...
    public static final String GET_ALL_ACTIVE = "SELECT owner, ownerName, expiry, cost, removed, type, contents " +
            "FROM `reforged_gts_trades` " +
            "WHERE expiry > ? AND removed = 0;";

    public static final String GET_ALL_PLAYER = "SELECT expiry, cost, removed, type, contents " +
            "FROM `reforged_gts_trades` " +
            "WHERE owner = ?;";
    ...
    public static final String UPDATE_REMOVED = "UPDATE `reforged_gts_trades` " +
            "SET removed = ? WHERE owner = ? AND expiry = ? AND cost = ? AND type = ?;";

    public static final String ADD_TRADE = "INSERT INTO `reforged_gts_trades`" +
            "(owner, ownerName, expiry, cost, removed, type, contents) VALUES (?, ?, ?, ?, ?, ?, ?);";

    public static final String REMOVE_TRADE = "DELETE FROM `reforged_gts_trades` " +
            "WHERE owner = ? AND expiry = ? AND cost = ? AND type = ?;";
