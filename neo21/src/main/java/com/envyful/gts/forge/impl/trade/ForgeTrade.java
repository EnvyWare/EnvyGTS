...
public abstract class ForgeTrade implements Trade {
    ...
    public boolean attemptPurchase(EnvyPlayer<?> player) {
        ...
        var bankAccount = parent.getBankAccountNow();
        ...
        if (NeoForge.EVENT_BUS.post(new TradePurchaseEvent((ForgeEnvyPlayer)player, this)).isCanceled()) {
            return false;
        }
        ...
        this.setRemoved().whenCompleteAsync((unused, throwable) -> {
            this.collect(player, null).thenApply(unused1 -> {
                ...
                NeoForge.EVENT_BUS.post(new PostTradePurchaseEvent((ForgeEnvyPlayer) player, this));
